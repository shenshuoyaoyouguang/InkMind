              ...
              <div class="config-content">
                ...
                <div class="config-section">
                  ...
                  <el-row :gutter="20">
                    <el-col :span="12">
                      <el-form-item>
                        <el-checkbox v-model="config.isDefault"
                          >设为默认模型</el-checkbox
                        >
                      </el-form-item>
                      <el-form-item>
                      ...
                      <el-form-item>
                        <el-checkbox v-model="config.streamMode"
                          >启用流式输出</el-checkbox
                        >
                      </el-form-item>
                    </el-col>
                ...
                <div class="config-actions">
                  ...
                  <el-button @click="duplicateConfig(config)"
                    >复制配置</el-button
                  >
...
const activeConfigs = computed(() => {
  return apiConfigs.value.filter((config) => config.enabled && config.apiKey)
    .length;
});
...
const connectedConfigs = computed(() => {
  return apiConfigs.value.filter((config) => config.status === "connected")
    .length;
});
...
const testAllConnections = async () => {
  const enabledConfigs = apiConfigs.value.filter(
...
const saveAllConfigs = async () => {
  const validConfigs = apiConfigs.value.filter(
    (config) => config.name && config.apiUrl,
  );
  ...
  // 保存到本地存储
  await storage.set("aiApiConfigs", apiConfigs.value);
...
onMounted(async () => {
  // 从本地存储加载配置
  const savedConfigs = await storage.get("aiApiConfigs");
  if (savedConfigs) {
    try {
      // 为现有配置添加unlimitedTokens字段
      apiConfigs.value = savedConfigs.map((config) => ({
        ...config,
        unlimitedTokens:
          config.unlimitedTokens !== undefined
            ? config.unlimitedTokens
            : config.maxTokens === null,
      }));
      if (apiConfigs.value.length > 0) {
        activeTab.value = apiConfigs.value[0].id;
