            ...
            <div class="about-content">
              <div class="app-info">
                ...
                <div class="app-details">
                  ...
                  <p><strong>更新时间：</strong>2026年5月18日</p>
                  <p><strong>开发者：</strong>InkMind Team</p>
                  <p>
                    <strong>描述：</strong
...
const exportAllData = () => {
  ...
  a.href = url;
  a.download = `InkMind-完整备份-${new Date().toISOString().split("T")[0]}.json`;
  a.click();
...
const exportNovels = () => {
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = `InkMind-小说数据-${new Date().toISOString().split("T")[0]}.json`;
  a.click();
  URL.revokeObjectURL(url);
...
const exportPrompts = () => {
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = `InkMind-提示词库-${new Date().toISOString().split("T")[0]}.json`;
  a.click();
  URL.revokeObjectURL(url);
...
const exportGenres = () => {
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = `InkMind-小说类型-${new Date().toISOString().split("T")[0]}.json`;
  a.click();
  URL.revokeObjectURL(url);
...
const exportSettings = () => {
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = `InkMind-系统设置-${new Date().toISOString().split("T")[0]}.json`;
  a.click();
  URL.revokeObjectURL(url);
      ...
      ElMessageBox.confirm(
        ...
        if (importOptions.value.includes("writingGoals")) {
          if (data.writingGoals) {
            localStorage.setItem(
              "writingGoals",
              JSON.stringify(data.writingGoals),
            );
            importCount++;
          } else if (data.goals) {
        ...
        if (importOptions.value.includes("settings") && data.settings) {
          if (data.settings.apiConfig) {
            localStorage.setItem(
              "api-config",
              JSON.stringify(data.settings.apiConfig),
            );
            importCount++;
          }
          if (data.settings.tokenUsage) {
            localStorage.setItem(
              "token-usage",
              JSON.stringify(data.settings.tokenUsage),
            );
            importCount++;
          }
        }
