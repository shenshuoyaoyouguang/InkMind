  ...
  <div class="corpus-manager">
    ...
    <div class="corpus-list-section">
      ...
      <div v-else class="corpus-list">
        <div v-for="item in paginatedCorpus" :key="item.id" class="corpus-item">
          <div class="corpus-header">
            ...
            <div class="corpus-actions">
              <el-button type="text" size="small" @click="editCorpus(item)">
    ...
    <!-- 查看全文对话框 -->
    <el-dialog v-model="showContentDialog" title="语料全文" width="700px">
...
const saveEdit = () => {
  ...
  const index = corpus.value.findIndex(
    (item) => item.id === editingCorpus.value.id,
  );
  if (index > -1) {
...
const deleteCorpus = async (id) => {
  try {
    await ElMessageBox.confirm("确定要删除这条语料吗？", "确认删除", {
