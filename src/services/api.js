...
class APIService {
    ...
    try {
      ...
      try {
            ...
            if (trimmedLine.startsWith("data: ")) {
              ...
              try {
                ...
                if (parsed.error) {
                  ...
                  throw new Error(
                    `API错误: ${parsed.error.message || "未知错误"}`,
                  );
                }
              } catch (e) {
        ...
        if (fullContent.length === 0) {
          // 内容为空时由调用方处理异常，此处不打印日志
        } else if (fullContent.length < 10) {
          // 内容过短时由调用方处理异常，此处不打印日志
        }
      } catch (streamError) {
  ...
  async generateOutline(theme, keywords, template) {
    return this.generateOutlineStream(theme, keywords, template);
  }
...
请直接输出大纲内容：`;
  ...
  async generateChapterContent(
    ...
    return this.generateChapterContentStream(
      chapterTitle,
      chapterOutline,
      previousContent,
      template,
...
请直接输出章节内容：`;
  ...
  async generateSummary(content, options = {}) {
    ...
    return await this.generateTextStream(
      prompt,
      {
...
生成内容：`;
  ...
  async generateGeneralContent(keywords, template, outline, wordLimit = 500) {
    return this.generateGeneralContentStream(
      keywords,
      template,
      outline,
      wordLimit,
        ...
        messages: [
          {
            role: "system",
            content:
              "你是一位专业的文学评论家和编辑，擅长分析各种类型的文章。请客观、专业地分析文章，给出建设性的评价和建议。",
          },
          {
            role: "user",
            content: prompt,
