class BillingService {
  ...
  recordAPICall(params) {
    try {
      ...
      console.log(
        `API调用记录：模型=${params.model}, 输入=${params.inputTokens}tokens, 输出=${params.outputTokens}tokens, 费用=¥${cost.toFixed(4)}`,
      );
  ...
  updateUsageStats(inputTokens, outputTokens, cost) {
    try {
      const stats = JSON.parse(
        localStorage.getItem("token_usage_stats") || "{}",
      );
  ...
  getTodayStats() {
    ...
    return {
      tokenCount: todayRecords.reduce(
  ...
  getUsageTrend(days = 7) {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const dayRecords = records.filter(
        (record) => new Date(record.timestamp).toDateString() === dateString,
      );

      trend.push({
        date: dateString,
        tokenCount: dayRecords.reduce(
          (sum, record) => sum + record.totalTokens,
          0,
        ),
  ...
  cleanOldRecords() {
    try {
      ...
      console.log(
        `清理了 ${records.length - filteredRecords.length} 条过期记录`,
      );
    } catch (error) {
  ...
  exportBillingData(format = "json") {
    ...
    if (format === "json") {
      ...
      // 简单的CSV格式
      let csv =
        "timestamp,type,model,inputTokens,outputTokens,totalTokens,cost,status\n";
